...
class LatentDiffusion(DDPM):
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        ...
        if isinstance(encoder_posterior, DiagonalGaussianDistribution):
            print("DDPM/get_first_stage_encoding: DiagonalGaussianDistribution")
            z = encoder_posterior.sample()
        elif isinstance(encoder_posterior, torch.Tensor):
            print("DDPM/get_first_stage_encoding: torch.tensor")
            z = encoder_posterior
        else:
    ...
    def get_learned_conditioning(self, c, inv=False):
        print("Get_learned_conditioning (exceptional )")
        # cond_stage_model=CLIP
        # print("Cond_stage_model",print(self.cond_stage_model))
    ...
    def forward(self, x, c, *args, **kwargs):
        #print("LatentDiffusion/forward",print(self.conditioning_key))
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key is not None:
            assert c is not None
            if self.cond_stage_trainable:
                print("DDPM Forward Case 1")
                c = self.get_learned_conditioning(c)
            if self.shorten_cond_schedule:  # TODO: drop this option
                print("DDPM Forward Case 2")
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        print("Diffusion Wrapper/Conditional key",self.conditioning_key)
        print("Diffusion Wrapper/diffusion model",print(type(self.diffusion_model)))
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None, encode=False, encode_uncon=False, decode_uncon=False, controller=None, inject=False):
        #print("DiffusionWrapper, forward",print(self.conditioning_key))
        if self.conditioning_key is None:
            ...
            #print("Diffusion Wrapper/forward/self.sequential_cross_attn",self.sequential_cross_attn) ==False
