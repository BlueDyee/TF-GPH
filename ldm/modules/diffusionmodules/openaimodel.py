...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, encode=False, encode_uncon=False, decode_uncon=False, controller=None, inject=False, layernum=0):
        for layer in self:
            #print("In timestepEbed Sequential")
            if isinstance(layer, TimestepBlock):
                #print("In timestepEbed Sequential/case 0")
                x = layer(x, emb)
                """
                if x.shape[0] == 4:
                    print("In timestepEbed Sequential/case 0-0")
                    x1 = layer(x[:2], emb)
                    x2 = layer(x[2:], emb)
                    x = th.cat([x1, x2], dim=0)
                else:
                    print("In timestepEbed Sequential/case 0-1")
                """ 
            elif isinstance(layer, SpatialTransformer):
                #print("In timestepEbed Sequential/case 1")
                ...
                #print("In timestepEbed Sequential/case 2")
                x = layer(x)
        return x, layernum
...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x, emb):
        #print("Resblock_forward 0",x.shape,emb.shape)
        if self.updown:
        ...
        if self.use_scale_shift_norm:
            ...
            #print("Resblock exception 2",h.shape,emb_out.shape)
            h = h + emb_out
            h = self.out_layers(h)
...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv):
        ...
        print("QKV ATTENTION LEGACY:",q.shape, k.shape, v.shape)
...
class QKVAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        print("QKV ATTENTION:",q.shape, k.shape, v.shape)
        scale = 1 / math.sqrt(math.sqrt(ch))
        weight = th.einsum(
            "bct,bcs->bts",
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, encode=False, encode_uncon=False, decode_uncon=False, controller=None, inject=False, **kwargs):
        ...
        layernum = 0
        #print("This Forward:",x.shape,emb.shape,timesteps.shape,context.shape)
        h = x.type(self.dtype)           
        for module in self.input_blocks:
            ...
            #print("This Forward self.input_blocks:",x.shape,emb.shape,timesteps.shape,context.shape,encode,encode_uncon,decode_uncon,controller)
            h, layernum = module(h, emb, context, encode=encode, encode_uncon=encode_uncon, decode_uncon=decode_uncon, controller=controller)
            hs.append(h)
            # print(layernum)
        ...
        h = h.type(x.dtype)
        #print("!!! Finish This Forward:",h.shape)
        if self.predict_codebook_ids:
