...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t, DPMencode=False, controller=None, inject=False, ref_init=None):
        ...
        #print("data_prediction_fn",x.shape, sigma_t.shape,noise.shape,alpha_t.shape)
        # 3-> (3,1,1,1)
    ...
    def dpm_solver_first_update(self, x, s, t, model_s=None, return_intermediate=False, DPMencode=False):
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            alpha_t=alpha_t.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            phi_1=phi_1.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            #print("Dpm sover_first update",sigma_t.shape,sigma_s.shape,alpha_t.shape,phi_1.shape,model_s.shape)
            x_t = (
                (sigma_t / sigma_s) * x
    ...
    def multistep_dpm_solver_second_update(self, x, model_prev_list, t_prev_list, t, solver_type="dpmsolver", DPMencode=False):
        ...
        r0 = h_0 / h
        r0=r0.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if solver_type == 'dpmsolver':
                ...
                sigma_t=sigma_t.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                sigma_prev_0=sigma_prev_0.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                alpha_t=alpha_t.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                phi_1=phi_1.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                #print("multistep/dpmsolver shape",sigma_t.shape, sigma_prev_0.shape, alpha_t.shape, phi_1.shape)
                x_t = (
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        device = x.device
        intermediates = []
        print("DPM_solver x.shape:",x.shape)
        with torch.no_grad():
            if method == 'adaptive':
                ...
                for step in tqdm(range(order, steps + 1), desc='DPM++ sampling (denoising) ZT-->Z0'):
                    t = timesteps[step].expand((x.shape[0]))
